...
For the test system, only the machine learning-based approach (C) was used since the unique diffraction patterns are not known in advance. 
...
The VADF image was created by applying a mask that blocked out the Al matrix reflections and then summing the intensity in each PED pattern. 
...
Illustration of defining template patterns by approach (B) manual selection, which involves manually selecting five regions with unqiue patterns based on the morphologies in the VADF image. The pre-processed SPED dataset was averaged over each region to yield the template patterns shown to the right. 
:::
...
To accomodate for this, overlap patterns were created by simply taking the average of each pair of dissimilar template patterns and rescaling the intensity. 
